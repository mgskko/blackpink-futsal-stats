...
const MOMVoting = ({ matchId }: MOMVotingProps) => {
  const { user } = useAuth();
  ...
  const { data: votes } = useQuery({
    queryKey: ["mom_votes", matchId],
    queryFn: async () => {
      const { data } = await supabase.from("mom_votes").select("*").eq("match_id", matchId);
  ...
  const handleVote = async () => {
    ...
    try {
      if (myVote) {
        await supabase.from("mom_votes").update({ voted_player_id: selectedPlayer }).eq("id", myVote.id);
      } else {
